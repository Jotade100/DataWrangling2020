...
library(igraph)
library(ggraph)
chat <- rwa_read("Chat de WhatsApp con Andrés Morales (Costa Rica).txt")
...
chat <- chat %>% filter(text != "<Multimedia omitido>" & !is.na(author))
...
to_remove <- c(stopwords(language = "en"),
               ...
               c(1:1000), c('éso', 'ésa', 'ésto', 'este', 'éste', 'eso', 'esto', 'ésta'))
...
chat %>%
  ...
  xlab("") +
  ggtitle("Diversidad de palabras o quien tira mas barrio") +
  coord_flip()
...
chat %>%
  ...
  ylab("") + xlab("") +
  coord_flip() +
  ggtitle(paste("Unique words of", unique(chat$author)[1]))
...
chat  %>% 
  ...
  filter(str_detect(chat$text,"Widor")) %>% 
  group_by(author) %>% 
  count(day) %>%  
  ggplot(aes(x = day, y = n, color = author)) +
  geom_line(stat = "identity") +
  ylab("") + xlab("") +
  ggtitle("Frecuencia de mensajes por dia (Donde se hable de Widor)")
...
paired_words_counts %>%  filter(n >= 7) %>%
